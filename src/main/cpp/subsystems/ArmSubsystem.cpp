...
void ArmSubsystem::Periodic() {
  ...
  SetTargetAngle(units::angle::degree_t{
      SmartDashboard::GetNumber("Arm Angle", GetAngle().value())});
  ...
  if (state == ArmStates::kArmOff) {
    ...
    SmartDashboard::PutNumber(
        "leftArmTr",
        leftArm.GetPosition().GetValue().value()); // print to Shuffleboard
...
void ArmSubsystem::ConfigArm() {
  ...
  armConfig.MotorOutput.Inverted = false;
  ...
  armConfig.Feedback.RotorToSensorRatio = kArmRotorToGearbox;
  armConfig.Feedback.SensorToMechanismRatio = kArmGearboxToMechanism;
